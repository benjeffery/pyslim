...
from .slim_metadata import default_slim_metadata
from .slim_metadata import is_current_version
from .slim_metadata import set_metadata_schemas
from .slim_metadata import set_tree_sequence_metadata
...
def _record_vacant_tables(tables):
    ...
    for n in nodes:
        ...
        if (n.metadata is not None) and (
            _is_chrom_vacant(k, n.metadata["is_vacant"]) and (f & tskit.NODE_IS_SAMPLE)
        ):
...
def generate_nucleotides(ts, reference_sequence=None, keep=True, seed=None):
    """
    Returns a modified tree sequence in which mutations have nucleotides assigned
    at random and (optionally) a reference sequence generated.
    ...
    sequence. If no reference sequence is given, the ``reference_sequence``
    property of ``ts`` is used if present; otherwise, independent uniformly
    random nucleotides are generated.

    SLiM stores the nucleotide as an integer in the mutation metadata, with -1
    meaning "not a nucleotide mutation". This method assigns nucleotides by
    iterating through each mutation and picking a random nucleotide uniformly
    from the three options that differ from the parental state (the derived
    state of the parental mutation, or the ancestral state if there is no
    parent). If ``keep=True`` (the default), mutations that already have a
    nucleotide (an integer 0-3 in metadata) are not modified.

    Technical note: in the case of stacked mutations, the SLiM mutation that
    determines the nucleotide state of the (tskit) mutation is the one with the
    largest ``slim_time`` attribute. This method tries to assign nucleotides so
    that each mutation differs from the previous state, but this is not always
    possible in certain unlikely cases.

    :param tskit.TreeSequence ts: The tree sequence to transform.
    :param bool reference_sequence: A reference sequence, or None to use an
        existing reference, or to randomly generate one.
    :param bool keep: Whether to leave existing nucleotides in mutations that
        already have one.
...
def _annotate_nodes_individuals(tables, age):
    ...
    for j, _ in enumerate(tables.individuals):
        if slim_ind[j]:
...
def _annotate_sites_mutations(tables):
    ...
    slim_time = tables.metadata["SLiM"]["tick"] - np.floor(
