...
class TestRecapitate(tests.PyslimTestCase):
    ...
    def test_root_mismatch_error(self):
        ...
        with pytest.raises(ValueError, match="Not all roots"):
            self.do_recapitate(ts, ancestral_Ne=10)
            ...
            for time in range(0, max_time_ago):
                ...
                if time == max_time_ago:
                    if remembered_stage == "early":
                        # if we set up the population in early there aren't
                        # individuals in first of the very first time step
                        ...
                        if ("everyone" in recipe or ind_time == 0) and (
                            ...
                            if slim_alive:
                                ...
                                if ts.metadata["SLiM"]["model_type"] == "WF":
                                    # SLiM records -1 but we return 0 in late and 1
                                    # in early
...
class TestHasIndividualParents(tests.PyslimTestCase):
    ...
    def test_pedigree_parents_everyone(self, recipe):
        # We can only guarantee to correctly reconstruct parents when everyone is
        # remembered: for instance, A selfs to produce B who selfs to produce C; if
        # A and C are present but B is not, and A is still alive, we will think that
        # A is C's parent. Or, suppose that X is the parent of Y, but we did not
        # remember X at or after the time that Y was born, so that although X is
        # alive at Y's birth, we don't know it, and so the parentage would not be
        # reported by `individual_parents()`.
        for _, ts in recipe["ts"].items():
            info = recipe["info"]
            with pytest.warns(FutureWarning):
                has_parents = pyslim.has_individual_parents(ts)
    ...
    def test_pedigree_parents(self, recipe):
        # Less strict test for consistency only: see caveats above in
        # test_pedigree_parents_everyone. In particular, we are only guaranteed to
        # have whole genomes for ALIVE or REMEMBERED individuals (not RETAINED), and
        # the same for parental genomes.
        info = recipe["info"]
        for _, ts in recipe["ts"].items():
            with pytest.warns(FutureWarning):
                has_parents = pyslim.has_individual_parents(ts)
        ...
        for _, ts in recipe["ts"].items():
            if ts.num_mutations > 0:
                ...
                if not has_nucleotides:
                    ...
                    assert isinstance(ts.reference_sequence.data, str)
        ...
        for _, ts in recipe["ts"].items():
            for _ in range(min(10, ts.num_sites)):
                ...
                for _ in range(10):
                    ...
                    ut = (
                        ts.node(node).time + (rtime - ts.node(node).time) * rng.random()
                    )
                    ...
                    for _ in range(100):
                        ...
                        if parent == tskit.NULL:
                            ...
                            c = pyslim.nucleotide_at(
                                ts, node, pos, ts.node(parent).time
                            )
                            assert b == c
...
class TestConvertNucleotides(tests.PyslimTestCase):
    ...
    def verify_converted_nucleotides(self, ts, cts):
        ...
        for s, ns in zip(ts.sites(), cts.sites()):
