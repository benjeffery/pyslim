...
class TestAnnotate(tests.PyslimTestCase):
    ...
    def test_warns_overwriting_mutations(self, helper_functions):
        ...
        with pytest.warns(Warning, match="already has.*metadata"):
            pyslim.annotate(ts, model_type="WF", tick=1)
    ...
    def test_annotate_mutations(self, helper_functions):
        for ts in helper_functions.get_msprime_examples():
            ...
            for j, x in enumerate(new_ts.mutations()):
                md = x.metadata
                assert np.isclose(
                    md["mutation_list"][0]["selection_coeff"], selcoefs[j]
                )
    ...
    def test_dont_annotate_mutations(self):
        ...
        pyslim.annotate_tables(
            tables, model_type="WF", tick=1, annotate_mutations=False
        )
    ...
    def test_annotate_empty(self):
        ...
        for x in ("population", "individual", "node", "site", "mutation"):
            assert (
                getattr(t, x + "s").metadata_schema == pyslim.slim_metadata_schemas[x]
            )
    ...
    def test_no_populations_errors(self, helper_functions, tmp_path):
        ...
        t = ts.dump_tables()
        t.populations.add_row(metadata={"name": "", "description": ""})
        ...
        pop = t.populations[-1]
        t.populations[-1] = pop.replace(
            metadata={"name": "not_slim", "description": ""}
        )
        ind = t.individuals[-1]
        md = ind.metadata
    ...
    def test_remaps_migration_table(self, helper_functions, tmp_path):
        ...
        restart_subpop_map = {"p5": 0, "p2": 1, "p0": 2}
        rts = helper_functions.run_slim_restart(
            {"default": ts},
            "restart_WF.slim",
            tmp_path,
            multichrom=False,
            WF=True,
            subpop_map=restart_subpop_map,
        )["default"]
        self.verify_remapping(ts, rts, restart_subpop_map)

    @pytest.mark.parametrize(
        "restart_name, recipe",
        restarted_recipe_eq("remove_subpop"),
        indirect=["recipe"],
    )
