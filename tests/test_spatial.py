...
class TestPopulationSize(tests.PyslimTestCase):
    ...
    def population_size_simple(self, ts, x_bins, y_bins, time_bins, **kwargs):
        ...
        # where pxiyjtk is the population size in [x_breaks[i], x_breaks[i + 1]) and
        # [y_breaks[j], x_breaks[j + 1]), averaged over each time point in
        # [t[k], t[k + 1]). Different offsets for different stages mirror
        # pyslim.individuals_alive_at code.
