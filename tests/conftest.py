...
class Outfiles:
    ...
    def __init__(self, out_dir):
        ...
        self._outfiles = [
            # Tree sequence output path
            self.Outfile(
                path=os.path.join(out_dir, "out.trees"),
                slim_name="TREES_FILE",
                post_process=self.load_ts,
                key="ts",
            ),
            self.Outfile(
    ...
    def __init__(self, out_dir, species):
        ...
        for sp in species:
            self._outfiles.extend(
                [
                    # Tree sequence output for this species
                    self.Outfile(
                        path=os.path.join(out_dir, f"{sp}_out.trees"),
                        slim_name=f"TREES_FILE_{sp}",
                        post_process=tskit.load,
                        key=f"ts_{sp}",
                    ),
                    # Pedigree information for this species
                    self.Outfile(
                    ...
                    # Mutation information for this species
                    self.Outfile(
...
def run_slim(recipe, out_dir, recipe_dir="test_recipes", species=None, **kwargs):
    ...
    log_path = os.path.join(out_dir, "SLiM_run_output.log")
    cmd_str = " ".join(cmd)
    print(f"Running {cmd_str}, outputting errors etc. to '{log_path}'")
    with open(log_path, "w") as out:
        retval = subprocess.call(cmd, stderr=subprocess.STDOUT, stdout=out)
    if retval != 0:
        errmsg = f"Could not run {' '.join(cmd)}"
        errline = False
        with open(log_path) as err:
            for line in err:
...
def recipe(request, tmp_path_factory, worker_id):
    """
    Most logic in this fixture is to check whether we are running as a single
    process or as a worker. If a worker, and this is the first run, we need to
    lock the process until the simulation has finished.
    """
